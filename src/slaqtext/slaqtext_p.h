...
public:
    ...
    SlackTextPrivate():
        selectionColor(QRgb(0xFFFF0080))
    ...
    bool allSelected() const {
        Q_Q(const SlackText);
        return !m_tp->extra->doc->toPlainText().isEmpty() && m_selstart == 0 && m_selend == (int)m_tp->extra->doc->toPlainText().length();
    }
    ...
    int end() const {
        Q_Q(const SlackText);
        return m_tp->extra->doc->toPlainText().length();
    }
    ...
    QString textBeforeSelection() const {
        Q_Q(const SlackText);
        return hasSelectedText() ? m_tp->extra->doc->toPlainText().left(m_selstart) : QString();
    }
    QString textAfterSelection() const {
        Q_Q(const SlackText);
        return hasSelectedText() ? m_tp->extra->doc->toPlainText().mid(m_selend) : QString();
    }
    ...
    QString realText() const {
        Q_Q(const SlackText);
        return m_tp->extra->doc->toPlainText();
    };
